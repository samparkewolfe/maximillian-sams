 ...
 */
#ifdef _MSC_VER
#pragma warning (disable : 4018) /* disable signed/unsigned pragma warnings */
#endif
...
public:
    maxiMFCCAnalyser():melFilters(nullptr), melBands(nullptr), dctMatrix(nullptr){};
    ~maxiMFCCAnalyser() {
        if (melFilters) {
            delete[] melFilters;
            delete[] melBands;
...
private:
    ...
    void calcMelFilterBank(double sampleRate, int numBins) {
        ...
        mel = minMel;
        for (unsigned int i=0;i<numFilters + 2;i++) {
        ...
        for (unsigned int filter = 1; filter < numFilters; filter++) {
